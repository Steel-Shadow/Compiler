...
后端采用简单稳健的栈式值分配策略：IR 结果通常存入栈槽，计算时加载到 `$t0-$t9`，完成后再写回。
...
当前后端优先保证正确性和可调试性，没有实现全局寄存器分配。所有 IR 临时值都有明确栈槽，因此生成代码较长，但定位问题比较直接。mem2reg 和局部 peephole 用来弥补一部分性能损失。

### 11.3 标量优先的优化策略

mem2reg 只处理非逃逸标量，不处理数组和指针对象。这样实现复杂度低，风险可控，也适合当前 MIPS 后端；后续可以在 IR 稳定后继续加入常量传播、死代码删除和寄存器分配。
